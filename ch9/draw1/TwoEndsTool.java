...
public class TwoEndsTool extends AbstractTool {
  ...
  public static final int CIRC = 3;
  public static final int TRIG = 4;
  ...
  public void addPointToShape(Point p) {
    if (canvas.mouseButtonDown) {
      ...
      case RECT:
        ...
        break; 
      case CIRC:
          drawCirc(g, xStart, yStart, canvas.x - xStart + 1, canvas.y - yStart + 1);  
          drawCirc(g, xStart, yStart, p.x - xStart + 1, p.y - yStart + 1); 
          break;         
      }
      canvas.x = p.x; 
      canvas.y = p.y; 
  ...
  public static void drawCirc(Graphics g, int x, int y, int w, int h) {
	    if (w < 0) {
	      x = x + w; 
	      w = -w; 
	    }
  ...
  public static void drawTrig(Graphics g, int x, int y, int w, int h) {
	    if (w < 0) {
	      x = x + w; 
	      w = -w; 
	    }
